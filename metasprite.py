...
def convert_metasprite(rom, frame_ptr, place_tbl_ptr):
    ...
    return metasprite, y_radius, x_radius

def print_metasprite(metasprite, y_radius, x_radius):
    print(f'    .byte ${((len(metasprite) > 0 and metasprite[0]['explode_idx'] != None) * 0x80) | y_radius:02X},${x_radius:02X}')
    for sprite in metasprite:
        ...
        if sprite['bg_priority']:
            attrs |= 0x20
        print(f'    .byte {f'${sprite['explode_idx']:02X},' if sprite['explode_idx'] != None else ''}${sprite['y'] & 0xFF:02X},${sprite['tile']:02X},${attrs:02X},${sprite['x'] & 0xFF:02X}')
    print('    .byte $80')
...
if __name__ == '__main__':
    ...
    for frame_num in range(0x6A):
        ...
        if ptr != 0x10000+rom.read_int(2):
            metasprite, y_radius, x_radius = convert_metasprite(rom, ptr, 0x186DF)
            canvas = {}
    ...
    for area_name, bank, pal_idxs, gfx_idxs, num_frames in area_data:
        ...
        for frame_num in range(num_frames):
            ...
            if ptr != bank+rom.read_int(2):
                metasprite, y_radius, x_radius = convert_metasprite(rom, ptr, place_ptr_tbl)
                canvas = {}
                add_to_canvas_from_spritemap(canvas, metasprite, gfx)
                width, height = bounding_box(canvas)
                image = to_image(canvas, -width, -height, width, height)
